...
final class AddDictionarySection extends JDialog
{
	...
	protected AddToDictionary ATD;
	public AddDictionarySection(AddToDictionary ATDTemp, final boolean language)
	{
		super(ATDTemp, true);
		...
		for(int i=0; i<ATD.sectionIndex.length; i++)
			sectionIndexTemp[i] = ATD.sectionIndex[i];
		...
		ageRadioButton[(ATD.ageIndex/5)-1].setSelected(true);
		...
		OKDictionarySectionButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					for(int i=0; i<ATD.sectionIndex.length; i++)
						ATD.sectionIndex[i] = sectionIndexTemp[i];
					
					ATD.ageIndex = ageIndexTemp;
					setVisible(false);
					dispose();
				}
				else
					JOptionPane.showOptionDialog(AddDictionarySection.this, variable[0], variable[1], JOptionPane.YES_OPTION, JOptionPane.ERROR_MESSAGE, null, OptionPaneYesLabel, OptionPaneYesLabel[0]);
			}
		});
