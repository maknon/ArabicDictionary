...
final class EditDictionarySection extends JDialog
{
	...
	protected EditDictionary ED;
	public EditDictionarySection(EditDictionary EDTemp, final boolean language)
	{
		super(EDTemp, true);
		...
		try
		{
			...
			ButtonGroup ageRadioGroup = new ButtonGroup();
			for(int i=0; rs.next(); i++)
			{
		...
		for(int i=0; i<ED.sectionIndex.length; i++)
			sectionIndexTemp[i] = ED.sectionIndex[i];
		...
		ageRadioButton[(ED.ageIndex/5)-1].setSelected(true);
		...
		OKDictionarySectionButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					for(int i=0; i<ED.sectionIndex.length; i++)
						ED.sectionIndex[i] = sectionIndexTemp[i];
					
					ED.ageIndex = ageIndexTemp;
					setVisible(false);
					dispose();
				}
				else
					JOptionPane.showOptionDialog(EditDictionarySection.this, variable[0], variable[1], JOptionPane.YES_OPTION, JOptionPane.ERROR_MESSAGE, null, OptionPaneYesLabel, OptionPaneYesLabel[0]);
			}
		});
