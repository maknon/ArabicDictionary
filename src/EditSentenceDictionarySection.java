...
final class EditSentenceDictionarySection extends JDialog
{
	...
	protected EditSentenceDictionary ESD;
	public EditSentenceDictionarySection(EditSentenceDictionary ESDTemp, final boolean language)
	{
		super(ESDTemp, true);
		...
		for(int i=0; i<ESD.sectionIndex.length; i++)
			sectionIndexTemp[i] = ESD.sectionIndex[i];
		...
		ageRadioButton[(ESD.ageIndex/5)-1].setSelected(true);
		
		final JPanel dictionarySectionButtonPanel = new JPanel(new GridLayout(1,24));
		...
		OKDictionarySectionButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					for(int i=0; i<ESD.sectionIndex.length; i++)
						ESD.sectionIndex[i] = sectionIndexTemp[i];
					
					ESD.ageIndex = ageIndexTemp;
					setVisible(false);
					dispose();
				}
				else
					JOptionPane.showOptionDialog(EditSentenceDictionarySection.this, variable[0], variable[1], JOptionPane.YES_OPTION, JOptionPane.ERROR_MESSAGE, null, OptionPaneYesLabel, OptionPaneYesLabel[0]);
			}
		});
